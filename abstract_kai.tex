...
While recent efforts have improved pair-wise inter-DC data transfers, they are insufficient for optimizing bulk-data multicast,
because they do not harness servers' capability to store-and-forward data as
well as the rich inter-DC overlay paths among geo-distributed DCs.
To harness these opportunities, we present {\em \name},
an application-level multicast overlay network that
sends inter-DC multicast traffic via
inter-DC overlay paths in a near-optimal manner.
At its core, \name {} {\em fully centralizes} the overlay network control, allowing a controller to maintain
...
decisions in near real time (e.g.,
every other second)
...
speedup over the provider's existing system and several widely used overlay routing baselines.
