\NEW{
\section{\newname: Dynamic Bandwidth Separation with Online Traffic}
\label{sec:dynamic}
...
\name performs well under strict network limitations, but will results in low bandwidth utilizations when online traffic reduces. The strict separation between online and offline traffic suffers from inefficient bandwidth usage in the hybrid deployment situations: when online traffic and offline traffic shares the same server I/O, bulk data transfer will not occupy any more bandwidth than the safety threshold even when online traffic is far below the reserved bandwidth (see \Section\ref{subsec:motivation:baseline}).

In this section, we present an enhanced version of \name, called \newname, which achieves dynamic bandwidth separation with online traffic in a real-time manner, by continuously predicting online traffic changes and automatically adjusting the scheduling decisions to fully utilize network bandwidth accordingly. In this way, \newname automatically adjust the scheduling results under different network conditions: if online traffic encounters its peak, \newname would shirk its occupies bandwidth to avoid congestions, while online traffic encounters its valley, \newname would aggressively use more bandwidth to make full use of the residual bandwidth.

To achieve this, \newname uses an online traffic prdiction algorithm, which identifies the changes of server bandwidth usage, and triggers re-scheduling to adjust bandwidth allocation. \newname's Network Change Monitor (Figure~\ref{fig:bayes}) reads the agent observations (bandwidth usage) and executes a change point detection algorithm. The Reconfig Engine is responsible for updating the changes based on agent observations and the detection results, reporting the changes to \name controller and enforcing the subsequent bandwidth adjustment.

\subsection{Approaches to Online traffic prediction}
