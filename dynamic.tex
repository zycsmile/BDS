...
\minor{ In \name, we implemented our customized $sliding-k$ algorithm based on \cite{adams2007bayesian} (with code can be found in \cite{BOCDcode}) into the Network Change Monitor. Specifically, we set an upper bound $\textsc{K}$ for the EWMA algorithm, $k$ gradually increases to $\textsc{K}$ when there is no change point, and will be reset to $0$ once a change point is detected, and then gradually increases to $\textsc{K}$ again. This improvement makes the $sliding-k$ more stable.
}
