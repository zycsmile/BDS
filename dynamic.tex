...
To achieve this, \newname adopts a customized online traffic prdiction algorithm, which identifies the changes of server bandwidth usage, and triggers re-scheduling to adjust bandwidth allocation to the bulk-data transfer. Figure~\ref{fig:bayes} shows the logical diagram of \newname's dynamic bandwidth separation. The Network Change Monitor reads the agent observations (bandwidth usage) and executes a change point detection algorithm. The Reconfig Engine is responsible for updating the changes based on agent observations and the detection results, reporting the changes to \newname controller and enforcing the subsequent bandwidth adjustment.
