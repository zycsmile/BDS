...
bandwidth \myfootnote{In \company, multiple services are mixed-deployed in the same DCs and even in the same servers, so the server bandwidth is shared by many services and the allocated available bandwidth for bunk data transfer is 20Mbps in general case}. This 30GB file was evenly stored in all these
640 servers.
