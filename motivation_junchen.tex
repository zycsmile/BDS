\section{A case for application-level inter-DC multicast overlay}
...
servers ({\em Type II}), or some combinations of the two types.
...
%link, and
The data is split into 6 6GB-blocks.
...
servers.
Instead, we show empirically study one example of bottleneck-disjoint overlay path in the wild, which consists of two overlay
...
below 0.8 or over 1.3 for about 30\% overlay pairs. These pairs are widely available, and we can create a new bottleneck-disjoint path by using any server $b$
as an overlay point.
...
link in two days during which there was a bulk data transfer at 11:00 on the 2nd day lasting for 6 hours. Figure~\ref{fig:lesson2} shows the total utilization of an inter-DC link over time. The blue line denotes the outgoing (outbound) bandwidth and the green line denotes the ingoing (inbound) bandwidth.
