...
%<<<<<<< HEAD
Table~\ref{table:rate} shows inter-DC multicast traffic
...
%}
\end{center}
\tightcaption{Inter-DC multicast traffic (replicating data from
...
It is known that the performance of multicast traffic can
...
While it is impractical to identify all bottleneck-disjoint overlay paths
...
To meet the need of rapid growth in multicast data,
...
Each DC has 640 servers, each with 20Mbps upload and download
bandwidth. This 30GB file will be evenly spread across all these 640 servers in the form of blocks. 
Theoretically, if servers select
the best source for all blocks, the ideal
completion time will be
$\frac{30\times 1024}{640\times 20Mbps \times 60s/min} = 41$
minutes. Figure~\ref{fig:motivation} shows the results under the existing
...
\begin{figure}[t]
  ...
  \tightcaption{CDF of the actual flow completion time of servers in the destination DCs.}
  \label{fig:motivation}
\vspace{-0.4cm}
\end{figure}
...
days. From this figure we can see that the bulk data transfer caused
...
\begin{figure}[t]
        ...
        \caption{The utilization of the inter-DC link. Bulk data transfer on the 2nd day caused obviously irregular traffic on the outgoing link.}
        \label{fig:lesson2}
%\vspace{0.1in}
\end{figure}
