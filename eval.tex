...
\begin{figure*}[t]
        ...
        \begin{subfigure}[b]{0.3\textwidth}
                ...
                \caption{CDF of algorithm prediction accuracy. Only in 1.6\% conditions BDS+ estimate bandwidth will be larger than real available bandwidth.}
...
We start the bulk data transfer at ***(time) on 27, Jan, 2019. Figure~\ref{fig:bdsplus:server} shows the CDF of the completion time on each destination server. We can see that the median completion time of \newname is *** minutes, even the straggler server finishes transfer within *** minutes, while most servers under \name takes more than *** minutes.

\mypara{Improvements over \name}
To make the results more general, we further conduct a series of experiments during different time periods, in other words, once per 15 minutes. We compare the completion time of both \newname and \name and show the results in Figure~\ref{fig:bdsplus:periods}. We can see that the improvements of \newname over \name changes with time, specifically, the improvements during midnight is significantly higher than that during the day, especially at ***(time), when online traffic is at its peak. These results show that \newname can make full use of the idle bandwidth that is not used by online traffic. Overall, the CDF of improvements is shown in Figure~\ref{fig:bdsplus:vs}, which means that \newname can bring at least 20\% improvement in more than 60\% cases.
