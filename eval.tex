...
\NEW{
\item \name can: \minor{(1)}. further complete inter-DC multicast 1.2 to 1.3 times faster with dynamic bandwidth separation, \minor{(2)}. predict the bandwidth utilization of online traffic with about 95\% accuracy, \minor{(3)}. increase bandwidth utilization when the online traffic is low, while reducing the bulk data transfer when online traffic bursts, \minor{(4)}. achieve near real-time scheduling with relatively low computational overhead (\Section\ref{subsec:evaluation:improvements}).
}
\end{packedenumerate}
\NEW{
...
trace-driven simulation to evaluate \name on a larger scale. \minor{ The simulation is not to reproduce the results of the above pilot deployment, but to provide evaluation results in large-scale scenarios, which is complementary to the pilot deployment.}
Specifically, we simulate the other two overlay multicast techniques using the
...
\minor{
The whole logic of \name can be summarized as follows: \name first obtains all the paths from the topology of \company network, and then conducts the dynamic bandwidth separation by predicting the online traffic on each link. Thus, \name obtains the residual bandwidth of all the paths, and therefore confirms the available links for the subsequent bulk data transfer. On this base, \name runs the scheduling and routing algorithm periodically to find overlay paths for those selected blocks. With the power of the bottleneck disjoint overlay paths, blocks can be transmitted on multiple paths simultaneously, and avoid going through bottleneck links. That's the key to accelerate inter-DC bulk data transfer.
}
\subsubsection{\name vs. \alg}
...
To generalize the finding, \minor{we pick three applications whose data
volumes are large (70~TB), medium (50~TB) and small (20~TB)}, and compare \name's and \alg's
...
in applications with larger data sizes. \minor{This is because \name adopts "rarest-first" strategy in the scheduling stage, which treats all blocks as the same no matter it belongs to a larger bulk data transmission or a smaller transmission, so there is a strong possibility that blocks from large bulk data transmissions will be scheduled earlier, resulting in greater improvement for those larger bulk data.}
...
simulation.
\minor{In the simulation, we set the inter-DC bandwidth to the range from $5TB$ to $25TB$, which is scaled down proportionally according to the real network.}
...
\begin{figure*}[t]
        ...
        \end{subfigure}
        \tightcaption{[\name vs. \bdsname] Further improvements from \bdsname.}
        \label{fig:bdsplus}
%\vspace{-0.4cm}
\end{figure*}
...
As existing solutions reserve fixed amount of bandwidth for online traffic according to the peak value (e.g., $20\%$), while real traces show that \minor{online traffic rarely reaches that peak and is far below that in most cases}. Thus, \name leverages dynamic bandwidth separation between online traffic and offline traffic, allowing offline traffic (bulk data transfer) to use more bandwidth when online traffic is below the threshold. \name achieves this by designing an online traffic prediction algorithm, and this section shows the results of improved performance by dynamic bandwidth separation. For easy description, we name the basic version with fixed bandwidth separation \bdsname, while the version with dynamic bandwidth separation \newname.
...
\subsubsection{Further improvements over \bdsname.}
