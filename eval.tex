...
\NEW{
\subsection{\name's performance improvement}
\label{subsec:evaluation:improvements}

\name reserves fixed size of bandwidth for online traffic (e.g., $20\%$), but lots of real traces show that online traffic rarely reaches the peak and is far below that in most cases. Thus, \name adopts dynamic bandwidth seperation between online traffic and offline traffic, allowing offline traffic (bulk data transfer) to use more bandwidth when online traffic is below the threshold. \name achieves this by designing an online traffic prediction algorithm, and this section shows the results of improved performance of \name.
