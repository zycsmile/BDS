...
failure scenarios.
\NEW{We then evaluate \name use trace-driven simulations, the results show that: \name further speed up the bulk data transfer by 1.2 to 1.3 times in the network where online and offline services are mixed deployed.
}
...
\NEW{
\item \name can make dynamic bandwidth separation to improve link utilization in the network where services are mixed deployed.
(\Section\ref{sec:overview},\ref{sec:dynamic}).
}
\item Demonstrating the practical benefits of \NEW{ and \name} by a real-world
