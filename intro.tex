...
\NEW{
In practice, there is always a fixed upper bound of available bandwidth for bulk-data multicast, because multicast overlay network shares the same inter-DC WAN with online latency-sensitive traffic. Existing solutions always reserve a fixed amount of bandwidth for the latency-sensitive traffic, according to its peak value. This guarantees the strict bandwidth separation, but the side affect is exposed especially when the online traffic is in its valley, wasting lot of bandwidth. To further improve link utilization, \newname deploys dynamic bandwidth separation that can predict online traffic and reschedule bulk-data transfer. In other words, \newname achieves dynamic bandwidth separation between bulk-data multicast and online traffic to further speed up data transfer.
%\jc{rescheduling what?}
%\newname addresses the real-time dynamic bandwidth separation problem by predicting the bandwidth utilization of online traffic and rescheduling.
}
...
failure scenarios.
\NEW{We then evaluate \newname use trace-driven simulations, the results show that: \newname further speed up the bulk data transfer by 1.2 to 1.3 times in the network where online and offline services are mixed deployed.
}
...
\NEW{
\item Presenting \newname, the advanced version of \name that can make dynamic bandwidth separation to improve link utilization in the mixed deployed network.
(\Section\ref{sec:overview},\ref{sec:dynamic}).
}
\item Demonstrating the practical benefits of \name \NEW{ and \newname} by a real-world
