...
\NEW{
To deploy \name in practice, one also needs to share the bandwidth dynamically 
and efficiently between bulk-data multicast (under centrallly controlled routing) and online traffic
(latency sensitive and mostly short flows).
\name addresses this practical challenge by by predicting the bandwidth utilization of online traffic and rescheduling 
...
failure scenarios.
\NEW{ To evaluate \newname, we use trace-driven simulations, the results show that: \newname further improves the bulk data transfer in the network where online and offline services are hybridly deployed.
}
...
\NEW{
\item Presenting \newname, the advanced version of \name that can work collaboratively with online services by making real-time dynamic bandwidth separations
