...
bulk-data multicast requests, and inefficiency on bulk-data transfer when latency-sensitive traffic is in its valley. Figure~\ref{fig:lesson2} shows the
...
These cases show that, an algorithm with dynamical interactions with latency-sensitive traffic would be more reasonable and efficient.
}
...
online traffic}.
\NEW{
\item {\em Dynamic bandwidth separation} can be helpful to improve link utilization by making full use of the remaining bandwidth of online services.
}
\end{packeditemize}
