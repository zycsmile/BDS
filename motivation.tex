...
DCs, \NEW{and by leveraging dynamic bandwidth separation} (\Section\ref{subsec:motivation:case-for}). Finally, we examine \company's current solution of inter-DC multicast (\alg), and draw
lessons from real-world incidents to inform the design of \name and \name
(\Section\ref{subsec:motivation:baseline}). \NEW{We conclude all these observations, which are based on
...
among 30+ geo-distributed DCs. (\Section\ref{subsec:motivation:observation})}
...
\NEW{
\mypara{Interaction with latency-sensitive traffic}
...
bulk-data multicast requests, and inefficiency on bulk-data transfer when latency-sensitive traffic is in ite valley. Figure~\ref{fig:lesson2} shows the
...
online traffic experienced over 30$\times$ delay inflation. Also, at 4:00-5:00am in the first day, near 50\% of the bandwidth was being wasted.
Therefore, an algorithm should be able to dynamically interact with latency-sensitive traffic.
}
...
\begin{figure}[t!]
        ...
        \tightcaption{The utilization of the inter-DC link in two days: \NEW{ The traffic valley on the 1st day results in nearly 50\% bandwidth waste. }Inter-DC bulk data transfer on the 2nd day caused severe interference on latency-sensitive traffic.}
        \label{fig:lesson2}
\vspace{-0.4cm}
\end{figure}
...
\noindent{\bf Limitation 3: Fixed bandwidth separation.}
As shown in Figure~\ref{fig:lesson2}, a fixed separation of link utilization would results in both excessive utilization and underutilization. Ideally, if we can make full use of the available bandwidth left by online traffic, then the green line would be infinitely close to the threshold. In this particular example, about 18.75\% bandwidth was wasted in those two days (while still caused excessive utilization case).
%multicast overlay network shares the same inter-DC WAN
%with latency-sensitive traffic. Despite using standard QoS techniques,
%and giving the lowest priority to bulk data transfers, we still see
%negative impacts on latency-sensitive traffic by bursty arrivals of
...
online traffic}.
\NEW{
\item {\em Dynamic bandwidth separation} can be achieved if: online traffic can be predicted accurately and rescheduling can be conducted accordingly.
}
\end{packeditemize}
