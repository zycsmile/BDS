...
online service providers is replicating bulk data from one DC to 
multiple DCs. 
While recent work has improved bulk-data
multicast by optimizing WAN performance, it is necessarily 
incomplete, as it falls short 
of exploring the abundant disjoint paths via servers in 
different DCs,
as well as servers' capability to store-and-forward data.
%Inspired by the early success of multicast overlay networks, 
To utilize these unexplored opportuntieis, we 
argue that an application-level multicast overlay network 
is needed to optimally deliver data via selectively picked 
overlay servers to fully exploit overlay paths.
Despite its promise, introducing multicast overlays to 
the context of inter-DC communication poses new challenges:
(1) the sheer number of servers and overlay paths renders
it untenable to maintain data delivery status at all servers 
in real-time, and make optimal overlay selections, and  
(2) it is vital to avoid any delay of latency-sensitive traffic 
caused by multicasting bulk data, since all traffic share the 
network resource. 
We present {\em \name}, a near-optimal multicast overlay network 
for bulk data multicast. 
\name is built on two design choices:
