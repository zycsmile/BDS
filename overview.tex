...
latency-sensitive traffic, we present {\em \newname} (based on {\em \name}), a {\em fully centralized} near-optimal application-level overlay network with dynamic bandwidth separation for inter-DC multicast.} Before presenting the details, we first highlight the intuitions behind the design choices, the challenges to make it practical.
...
across overlay servers. In contrast, if any network changes are detected, it is simpler to
allocate bandwidth of each data transfer by controlling the
sending rate at all servers in a centralized fashion (nomatter to reserve more bandwidth when traffic burst or to reduce transfer when traffic valley)
