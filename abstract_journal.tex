...
\NEW{To speeds up the computation of the control algorithm, \name decouples the origin algorithm into selection of overlay paths and scheduling of data transfers, so that each can be optimized efficiently. This enables \name to update overlay routing decisions at high scale, multicasting hundreds of TB data over tens of thousands of overlay paths. Furthermore, so that \name can be deployed  together with online services in a dynamic network with high workload churns, we present an enhanced version called \newname that can realize dynamic bandwidth separation by predicting online traffic and then adjust bandwidth allocation accordingly.
}
