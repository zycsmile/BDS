...
On one hand, this decoupling significantly reduces the computational overhead of the centralized controller. As the scheduling stage selects a subset of blocks and only these selected blocks will be routed and transferred in the following routing stage, the searching spaces are thus significantly reduced for the subsequent routing procedure, speeding up the execution of the centralized algorithm.
...
selects a subset of blocks to be transferred in each cycle and 
outputs the transmission order $\overrightarrow{o_B}$ for all blocks.
...
Given the objects that the scheduling step selects to transfer
($\overrightarrow{o_B}$), the routing step then decides for each data block $B$, the source $s_{B}^*$, the path $p_{B}^*$, and bandwidth allocation of the path $f^*_{B,p_{B}^*}$.
...
This approximation closely resembles \name's routing step as \name also splits data into tens of thousands of fine-grained data blocks (though not infinitesimally), and it is possible to solve this problem efficiently by a linear programming (LP) relaxation commonly used in the maximum concurrent flow (MCF) problem~\cite{garg2007faster,reed2012traffic}. 
