...
10,000 overlay paths and 100,000s objects.
...
We start with the basic formulation of the overlay routing problem with fixed network capacity and multicast transfers, and then extending it to handle dynamic changes in network performance and request arrivals.
...
\item Bandwidth constraint. The allocated bandwidth on path $p_\lambda$ should be the minimum of three parameters: the capacity $c(p_\lambda)$, source server upload rate $R_{up}(s)$, destination server download rate $R_{down}(d)$.
\begin{equation}
%\begin{split}
...
\item Path selection constraint: Only one path will be chosen for a particular block.
...
($\overrightarrow{o_B}$), and the subsequent routing procedure determines the routing paths ($p_{B}^*$) and allocated bandwidth ($f^*_{B,p_{B}^*}$) to these blocks.
...
The key to avoid introducing degradation in this procedure is to make sure that the optimal result is still retained in the reduced searching space, in other words, the optimal result is not in the space that is pruned by scheduling procedure. We claim that the optimal scheduling result is yield from the situation that all the blocks are balanced duplicated, thus the key concern is to balance the duplication of all blocks. So here \name prunes the searching space that will not result in balanced duplications and selects the least duplicated blocks to transmit first. In this way, the optimal result will thus be retained in the reduced searching space.
