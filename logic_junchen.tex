...
$10^4$s of overlay paths and $10^5$s of objects.
...
The key idea is to {\em decouple} the problem into two steps,
scheduling (i.e., which data blocks to be sent)
and routing (i.e., to which destination through which path)
 (\Section\ref{subsec:logic:separation}).
Thus, \name can solve both problems efficiently and near-optimally with proved guarantees
...
We start with the basic formulation of the overlay routing problem with fixed network capacity and multicast transfers, before extending it to handle dynamic changes in network performance and request arrivals.
...
many smaller fixed-sized {\em blocks}, and the $i$th block of the $j$th task is denoted as $B_{i,j}$ (By default, the block size is 2M, for efficient transmission and acceptable calculation overhead).
...
\name should be able to react to the changing network conditions by re-configuring the above formulation (by default, \name updates network information and resolves the problem every 3 seconds, and the cycle length is evaluated in \Section\ref{subsubsec:evaluation:depth}). Unfortunately, the origin problem becomes considerably complex and hard to solve during re-formulation. This is because the number of potential sources and paths grow exponentially with the duplication status and becomes impossible to find out the optimal transmission in such searching space. To make it solvable, \name decouples the problem into two steps and tries to find the optimal solution for each step.
...
($\overrightarrow{o_B}$), and the subsequent routing procedure determines the routing paths ($p_{B}^*$) and the allocated bandwidth ($f^*_{B,p_{B}^*}$) to these blocks.
