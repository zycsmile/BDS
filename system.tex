...
\item On receiving the updates from all Agents and the \minor{Network
...
\name's dynamic bandwidth separation also takes advantages of the centralized logic of \bdsname. The traditional
...
environments~\cite{wang2017toward}. \name, in contrast, dynamically predicts the
bandwidth usage of latency-sensitive applications, and calculates the
residual bandwidth that can be allocated to inter-DC multicast. Finally, note that \name optimizes
the application-level overlay, thus is complementary to
